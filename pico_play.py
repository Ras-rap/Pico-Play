...
class MusicPlayer:
    def __init__(self, songString='0 D4 8 0', looping=True, tempo=3, duty_cycle=58982 , pin=None, pins=[digitalio.DigitalInOut(GP0)]):
        self.tempo = tempo
        ...
        if pins is None:
            pins = [board.GP0]  # Default pin if pins list is not provided

        if pin:
            pins = [pin]

        for pin in pins:
            pwm = pwmio.PWMOut(pin, frequency=440, duty_cycle=0, variable_frequency=True)  # Default frequency and duty cycle, can be changed later
        ...
        if (not self.stopped):
            ...
            if (self.timer % self.tempo == 0):
                ...
                for pwm in self.pwms:
                    if (i >= len(self.playingNotes)):
                        ...
                        #Play note
                        if hasattr(pwm, 'duty_u16'):
                            pwm.duty_u16 = self.duty_cycle
                        else:
                            pwm.duty_cycle = self.duty_cycle
                        pwm.frequency = tones[self.playingNotes[i]]
                    i = i + 1
            ...
            if (len(self.playingNotes) > len(self.pwms)):
                ...
                if hasattr(p, 'duty_u16'):
                    p.duty_u16 = self.duty_cycle
                else:
                    p.duty_cycle = self.duty_cycle

                if (self.arpnote > len(self.playingNotes)-len(self.pwms)):
                    self.arpnote = 0
                self.pwms[len(self.pwms)-1].frequency = tones[self.playingNotes[self.arpnote+(len(self.pwms)-1)]]
